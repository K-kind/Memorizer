...
describe 'ユーザー新規登録、ログイン機能', type: :system, js: true do
  ...
  describe '新規登録' do
    ...
    context '有効な情報' do
      ...
      # let(:valid_user) { User.find_by(email: 'user@memorizer.tech') }

      it 'メール発送通知が表示される' do
        expect(page).to have_content '仮登録完了メールを送信しました。ご確認ください。'
      end

      it 'ユーザーと管理者にメールが発送される' do
        sleep 3
      ...
      #   expect(User.find_by(email: 'user@memorizer.tech').activated?).to be_falsey
      # end
      ...
      #   visit edit_account_activation_path('invalid token', email: valid_user.email)
