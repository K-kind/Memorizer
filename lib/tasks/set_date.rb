...
User.where('is_test_user = ? AND name != ?', true, 'テスト管理ユーザー').each do |user|
  ...
  user.learned_contents.where(is_test: true).each_with_index do |learned_content, index|
    if index <= 2
      learned_content.update!(calnedar_id: calendar_7days_ago.id, till_next_review: 3)
      learned_content.review_histories.update_all(calnedar_id: calendar_4days_ago)
    else
      # till_next_reviewは1時間おきに0にされている
      learned_content.update!(calnedar_id: calendar_1day_ago.id)
    end
  end
