...
User.where('is_test_user = ? AND name != ?', true, 'テスト管理ユーザー').each do |user|
  ...
  test_admin.learned_contents.each.with_index(1) do |original_content, index|
    ...
    learned_content = user.learned_contents.create!(
      ...
      calendar_id: user.calendars.find_or_create_by!(calendar_date: Time.zone.today - 1).id,
    ...
    if index.in?(1..5)
      learned_content.update!(
        calendar_id: user.calendars.find_or_create_by!(calendar_date: Time.zone.today - created_ndays_ago).id,
      ...
      learned_content.review_histories.create(
        ...
        calendar_id: user.calendars.find_or_create_by!(calendar_date: Time.zone.today - reviewed_ndays_ago).id,
  ...
  # 最初に作られたカレンダーは、日付を古くしておく
  user.calendars.update_all(created_at: Time.zone.yesterday)
  ...
  # template
  if template = user.learn_templates.last
    template.update!(
  ...
  # consulted_word
  user.consulted_words.destroy_all
